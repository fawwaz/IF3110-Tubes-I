...
else
{
    ...
    if ($target==SERVER_ROOT."/controllers/.php") {
        // artinya ini home page
        echo "home";
...
if(isset($getVars["action"])){
    $function=substr($getVars["action"],1,strlen($getVars["action"]));
    $function_clean=substr($function,0,-1);    
    $controller->$function_clean($getVars);
}else{
